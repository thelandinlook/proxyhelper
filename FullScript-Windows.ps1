...
if (Test-Path $config_file_path) {
    ...
    $drive = Read-Host "Would you like to use the previous drive? (hit ENTER for yes or enter a new drive)"
    ...
    if ([string]::IsNullOrWhitespace($dest_directory)) {
        $dest_directory = $prev_dir
    }
} else {
    $drive = Read-Host "Enter the drive letter (without colon) containing the 'PRIVATE\M4ROOT' folders"
...
foreach ($src_folder in $src_folders.GetEnumerator()) {
    ...
    try {
        Get-ChildItem -Path $src_folder.Value | ForEach-Object {
            $new_name = $_.BaseName
            if ($_.Directory.Name -eq 'SUB') {
                $new_name = $new_name -replace 'S03'
            }
            $dest_file = Join-Path $dest_folder "$new_name$($_.Extension)"
